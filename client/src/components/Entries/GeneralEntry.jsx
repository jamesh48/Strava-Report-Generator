...
export default (props) => {
  const { style, no, entry, sport, format, showIndividualEntry } = props;
  const { innerEntry, entryTitle, speed, generalEntry, specialEntry, entry1, entry2, entry3 } = style;
  const m2y = 1.094;
  const mps2kph = 3.6;

  const pastTense = sport === 'Walk' ? 'Walked-' : sport === 'Swim' ? 'Swam-' : sport === 'Ride' ? 'Rode-' : sport === 'Run' ? 'Ran' : 'traveled-';
  ...
  return (
    <div id={Number(no) === 0 ? entry1 : Number(no) === 1 ? entry2 : Number(no) === 2 ? entry3 : null} className={innerEntry}>
      <div className={(Number(no) >= 0 && Number(no) <= 2) ? `${generalEntry} ${specialEntry}` : generalEntry}>
        <a className={entryTitle} data-testId={entry.id} href='' onClick={showIndividualEntry}>{entry.name}</a>
        {format !== 'avgypace' ?

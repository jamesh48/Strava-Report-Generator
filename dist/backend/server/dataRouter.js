...
dataRouter.use(async (req, res, next) => {
  if (!req.session.athleteId) return (0, sendErrorCodes_1.send400)(res, "No Cookied User!!!");
  console.log("Cookied user");
  try {
    const { accessToken, expiresAt } = await (0, serverUtils_2.getCurrCredentials)(
    ...
    if (now > later) {
      ...
      const newAccessToken = await (0, serverUtils_1.refreshAccessToken)(
        apiConfigs_js_1.getRefreshedAccessTokenConfig,
        req.session.athleteId,
      );
...
dataRouter.get("/allEntries", async ({ currentAccessToken }, res) => {
  res.send(
    (await (0, controllers_1.getAllUserActivities)(currentAccessToken)).sort(
...
dataRouter.post("/addAllActivities", async ({ currentAccessToken }, res) => {
  ...
  (0, serverUtils_1.recurseResults)(
    (0, apiConfigs_js_1.getAllEntriesConfig)(currentAccessToken),
    [],
    callback,
  );
});
